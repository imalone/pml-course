...
From exploratory analysis it is apparent that there is a high level of interaction between the different variables.
...
#  apply(data,1,function(x){predict(modlist[[x['user_name']]],x)})
preds <- character(nrow(data))
for (user in unique(data$user_name)) {
  #preds[[paste0("user_name",user)]] <- predict(modlist[[user]],data)
  usersel <- data$user_name==user
  preds[usersel] <- as.character(predict(modlist[[user]],data[usersel,]))
}
#preds<-as.data.frame(preds,stringsAsFactors =FALSE)
#selectmat <- model.matrix(~user_name-1,data)

#preds <- preds[selectmat[,colnames(preds)]>0]
#preds<-as.factor(preds)
#levels(preds)<-levels(user_list)
preds
}

modknn<-train(classe~.,method="knn",data=training,trControl=ctrlBoot)
timeGBMb<-system.time(modFitGBMb<-train(classe~.,method="gbm",data=training,trControl=ctrlBoot,verbose=FALSE))
